(function ($) {
	...
	function PeMetaboxConditional(target, conf) {
		...
		var fields = {};
		var values,options,parsed = conf.options ? conf.options : JSON.parse(target.attr("data-options"));
		var state,name,value,fname,field;
		
		for (name in parsed) {
			options = parsed[name];
			field = target.find("[id=%0_%1]".format(id,name));
			field = field.length > 0 ? field : target.find("[id=%0_%1_]".format(id,name));
			field = field.length > 0 ? field : target.find("[id^=%0_%1_]".format(id,name));
			controls[name] = {
				target : field,
				values : {}
			...
			for (value in options) {
				...
				for (state in values) {
					...
					while ((fname = field.pop())) {
						if (!fields[fname]) {
							fields[fname] = target.find("[id=%0_%1]".format(id,fname)).closest("div.option");
							if (fields[fname].length === 0) {
		...
		function change(e) {
			...
			for (name in controls) {
				...
				value = control.target.length > 1 ? control.target.filter(":checked").val() : control.target.val();
				i = actions.length;
