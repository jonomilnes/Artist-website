jQuery(document).ready(function($) {
	...
	var projType = $('.pe-theme-project-type input');
	var builder = $('#pe_theme_meta_builder');
	var	mboxes = $(".pe_mbox");
	...
	function ptChange(e) {
		var selected = projType.filter(':checked').val();
		editor[selected === "builder" ? "hide" : "show"]();
		builder[selected === "builder" ? "show" : "hide"]();
		body[selected === "builder" ? "addClass" : "removeClass"]("pe-theme-page-builder-active");
		$(window).trigger("resize");
	}
	...
	if (projType.length > 0) {
		projType.change(ptChange).triggerHandler("change");
	}
});
