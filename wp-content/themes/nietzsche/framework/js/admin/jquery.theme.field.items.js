...
(function ($) {
	...
	function PeFieldItems(target, conf) {
		...
		function getData() {
			var row = 0;
			var data = [];
			tbody.find("> div").each(function (idx) {
				data[row] = {};
				var i,el,els = tbody.find(this).find("input, select, textarea");
				for (i=0;i<els.length;i++) {
					el = tbody.find(els[i]);
					data[row][el.attr('data-name')] = el.val();
				}
				row++;
			});
			return data;
		...
		function addValue(value) {
			...
			for (i = 0;i<fields.length;i++) {
				...
				case "textimg":
					...
					break;
				case "img":
					fieldTemplate = '<div><input id="pe_fields_item_field_%0" name="%1[%5][%2]" data-name="%2" type="text" value="%3" %4 data-markup="no"><a class="add-inline" title="Add Image" href=""></a></div>'.format(
						id,
						name,
						field.name,
						fval,
			...
			for (i=0;i<upfields.length;i++) {
				jel = jQuery("#%0".format(upfields[i]));
				jel.peFieldUpload({markup:jel.attr('data-markup') != 'no',text:'Set Layer Image'});
			}
