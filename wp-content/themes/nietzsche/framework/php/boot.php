...
// framework location
define('PE_FRAMEWORK',(function_exists('realpath') ? realpath(get_template_directory()) : get_template_directory()).'/framework');
...
function esc__pe($text) {
	print $text; // used for content already sanitized upon saving which doesn't need late escaping
}
...
if (defined("PE_EXPORT_DEMO_CONTENT")) {
	...
	add_action("export_filters",array(peTheme(),"export_filters"));
}
