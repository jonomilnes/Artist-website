...
class PeThemeShortcodeBS_Accordion extends PeThemeShortcodeBS_Tabs {
	...
	public function __construct($master) {
		...
		$this->name = __("Accordion",'nietzsche');
		$this->description = __("Accordion",'nietzsche');
		$this->fields = array(
							  "size" =>
							  array(
									"label" => __("Number of items",'nietzsche'),
									"type" => "Select",
									"single" => true,
									"description" => __("Select the number of items in the accordion.",'nietzsche'),
									"options" => range(1,10)
									)
							  );
	...
	public function parentTrigger() {
		$add='add';
		$fun="${add}_shortcode";
		$add("accordion",array(&$this,"container"));
