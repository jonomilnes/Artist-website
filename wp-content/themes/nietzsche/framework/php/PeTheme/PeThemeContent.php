...
class PeThemeContent {
	...
	public function filter($tax,$sep = "",$aclass="label",$template = '%s') {
		...
		if (is_array($terms) && count($terms) > 0) {
			$buffer = apply_filters("pe_theme_filter_item",sprintf('<a class="%s active" data-category="" href="#">%s</a>%s',$aclass,__("All",'nietzsche'),"$sep\n"),$aclass,"",__("All",'nietzsche'));
	...
	public function current($add = 0) {
		...
		esc__pe($this->wpq->current_post + $add);
	}
	...
	public function get_style() {
		...
		if ( count( $args ) > 1 ) {
			...
			foreach ( $args as $arg ) {
				...
				$arg = explode(':',$arg);
				$prop = empty( $arg[0] ) ? '' : $arg[0];
				$key = empty( $arg[1] ) ? '' : $arg[1];
					...
					if ( ! empty( $data->$key ) || $data->$key === '0' ) {
						...
						switch ( $prop ) {
							...
							case 'background-image':
								$template = 'background-image:url(\'%s\');';
								break;
						...
						$style .= sprintf($template,$data->$key);

					}

				} else if ( ! empty( $data->$prop ) ) {

					$style .= sprintf('%s;',rtrim($data->$prop,';'));

				}

			}

			$style = empty( $style ) ? '' : sprintf( 'style="%s"', esc_attr( $style ) );
	...
	public function the_content_more_link_filter( $link, $more_link_text ) {
		...
		//$link = sprintf('&nbsp;<a href="%s#more-%s" class="read-more">%s</a>',get_permalink(),$GLOBALS["post"]->ID,__("more",'nietzsche'));

		$link = sprintf( '&nbsp;<a href="%s" class="read-more">%s</a>', get_permalink(), $more_link_text === translate( '(more&hellip;)', 'default' ) ? __("more",'nietzsche') : $more_link_text );
	...
	public function breadcrumbs($sep = ' / ') {
		...
		$links[] = sprintf($template,'home',home_url(),__('Home','nietzsche'));
		if (is_home()) {
			$links[] = sprintf($template,'blog','#',__('Blog','nietzsche'));
		} elseif (is_category()) {
	...
	public function linkPages() {
		wp_link_pages(array('before' => '<div class="page-links">' . __('Pages:','nietzsche'), 'after' => '</div>'));
	}
	...
	public function get_the_archive_title() {
		...
		if ( function_exists( 'get_the_archive_title' ) ) {
			...
			if ( is_category() ) {
				$title = sprintf( __( 'Category: %s' ,'nietzsche'), single_cat_title( '', false ) );
			} elseif ( is_tag() ) {
				$title = sprintf( __( 'Tag: %s' ,'nietzsche'), single_tag_title( '', false ) );
			} elseif ( is_author() ) {
				$title = sprintf( __( 'Author: %s' ,'nietzsche'), '<span class="vcard">' . get_the_author() . '</span>' );
			} elseif ( is_year() ) {
				$title = sprintf( __( 'Year: %s' ,'nietzsche'), get_the_date( __( 'Y' ,'nietzsche') ) );
			} elseif ( is_month() ) {
				$title = sprintf( __( 'Month: %s' ,'nietzsche'), get_the_date( __( 'F Y' ,'nietzsche') ) );
			} elseif ( is_day() ) {
				$title = sprintf( __( 'Day: %s' ,'nietzsche'), get_the_date( __( 'F j, Y' ,'nietzsche') ) );
			} elseif ( is_tax( 'post_format' ) ) {
				if ( is_tax( 'post_format', 'post-format-aside' ) ) {
					$title = __( 'Asides' ,'nietzsche');
				} elseif ( is_tax( 'post_format', 'post-format-gallery' ) ) {
					$title = __( 'Galleries' ,'nietzsche');
				} elseif ( is_tax( 'post_format', 'post-format-image' ) ) {
					$title = __( 'Images' ,'nietzsche');
				} elseif ( is_tax( 'post_format', 'post-format-video' ) ) {
					$title = __( 'Videos' ,'nietzsche');
				} elseif ( is_tax( 'post_format', 'post-format-quote' ) ) {
					$title = __( 'Quotes' ,'nietzsche');
				} elseif ( is_tax( 'post_format', 'post-format-link' ) ) {
					$title = __( 'Links' ,'nietzsche');
				} elseif ( is_tax( 'post_format', 'post-format-status' ) ) {
					$title = __( 'Statuses' ,'nietzsche');
				} elseif ( is_tax( 'post_format', 'post-format-audio' ) ) {
					$title = __( 'Audio' ,'nietzsche');
				} elseif ( is_tax( 'post_format', 'post-format-chat' ) ) {
					$title = __( 'Chats' ,'nietzsche');
				}
			} elseif ( is_post_type_archive() ) {
				$title = sprintf( __( 'Archives: %s' ,'nietzsche'), post_type_archive_title( '', false ) );
			} elseif ( is_tax() ) {
				$tax = get_taxonomy( get_queried_object()->taxonomy );
				/* translators: 1: Taxonomy singular name, 2: Current taxonomy term */
				$title = sprintf( __( '%1$s: %2$s' ,'nietzsche'), $tax->labels->singular_name, single_term_title( '', false ) );
			} else {
				$title = __( 'Archives' ,'nietzsche');
			}
