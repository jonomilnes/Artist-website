...
class PeThemeGallery {
	...
	public function cpt() {
			...
			PeGlobal::$config["taxonomies"] = 
				array(
					  ...
					  array(
							'attachment',
							array(
								  //'label' => __('Media Tag','nietzsche'),
								  "labels" => 
								  array(
										'name' 				=> __('Media Tags','nietzsche'),
										'singular_name' 	=> __('Media Tag','nietzsche'),
										'search_items' 		=> __('Search Media Tags','nietzsche'),
										'popular_items' 	=> __('Popular Media Tags','nietzsche'),		
										'all_items' 		=> __('All Media Tags','nietzsche'),
										'parent_item' 		=> __('Parent Media Tag','nietzsche'),
										'parent_item_colon' => __('Parent Media Tag:','nietzsche'),
										'edit_item' 		=> __('Edit Media Tag','nietzsche'), 
										'update_item' 		=> __('Update Media Tag','nietzsche'),
										'add_new_item' 		=> __('Add New Media Tag','nietzsche'),
										'new_item_name' 	=> __('New Media Tag Name','nietzsche')
										),
								  'hierarchical' => false,
								  'sort' => true,
								  'show_ui' => true,
		...
		$cpt =
			array(
				  'labels' => 
				  array(
						'name'              => __("Galleries",'nietzsche'),
						'singular_name'     => __("Gallery",'nietzsche'),
						'add_new_item'      => __("Add New Gallery",'nietzsche'),
						'search_items'      => __('Search Galleries','nietzsche'),
						'popular_items' 	  => __('Popular Galleries','nietzsche'),		
						'all_items' 		  => __('All Galleries','nietzsche'),
						'parent_item' 	  => __('Parent Gallery','nietzsche'),
						'parent_item_colon' => __('Parent Gallery:','nietzsche'),
						'edit_item' 		  => __('Edit Gallery','nietzsche'), 
						'update_item' 	  => __('Update Gallery','nietzsche'),
						'add_new_item' 	  => __('Add New Gallery','nietzsche'),
						'new_item_name' 	  => __('New Gallery Name','nietzsche')
						),
				  'public' => true,
				  'has_archive' => false,
	...
	public function pe_theme_metabox_config_gallery() {
		...
		$galleryTypes[__("Direct Upload",'nietzsche')] = "upload";
		$mtagsDecription = "";

		if ($mtags) {
			$galleryTypes[__("Media Tag (any)",'nietzsche')] = "any";
			$galleryTypes[__("Media Tag (all)",'nietzsche')] = "all";
			$mtagsDecription = __("<strong>Media Tag</strong> includes all the already uploaded images that match the selected media tags. See the help documentation for an explanation of Media Tags",'nietzsche');
		}		
			...
			array(
				  ...
				  array(
						...
						array(
							  ...
							  "type" => "Select",
							  "description" => __("<strong>Direct upload</strong> lets you add images by dragging and dropping them directly from your computer.<br/>",'nietzsche').$mtagsDecription,
						...
						array(
							  ...
							  "description" => __("<strong>Newest First</strong> display recent uploaded images first.<br/><strong>Manual</strong> drag and drop sort.",'nietzsche'),
							  "type" => "RadioUI",
							  "options" => Array(__("Newest First",'nietzsche')=>"auto",__("Manual",'nietzsche')=>"custom"),
							  "default" => "auto"
							  ),
						"tags" =>
						array(
							  "label" => __("Media Tags",'nietzsche'),
							  "type" => "Tags",
						...
						"images" =>
						array(
							  "label" => __("Upload Gallery Images",'nietzsche'),
							  "description" => __("Add one or more media tags",'nietzsche'),
							  "type" => "DropUpload",
							  "default" => ""
							  )
						)
	...
	public function mediaTagsDropdown($selected) {
		wp_dropdown_categories(
							   array(
									 "show_option_all" => __("All",'nietzsche'),
									 "show_option_none" => __("Not Tagged",'nietzsche'),
									 "taxonomy"=>PE_MEDIA_TAG,
									 "name"=>PE_MEDIA_TAG,
									 "show_count"=>1,
									 "selected"=>$selected
		...
		if (wp_script_is("media-editor")) {
			if (function_exists("wp_enqueue_media")) {
				...
				wp_localize_script("pe_theme_media_filter","pe_theme_media_filters",
								   array(
										 "all" => __("All media tags",'nietzsche'),
	...
	public function option() {
		...
		if (count($posts) > 0) {
			...
			$options = array(__("No galleries defined, please create one",'nietzsche')=>-1);
		}
		return $options;
	}
	...
	public function &get($id) {
		...
		if (is_array($id)) {
			...
			$cid = "custom-".implode(",",$id);
			if (isset($this->cache[$cid])) return $this->cache[$cid];
			$post = new StdClass();
			$post->id = sprintf("custom-gallery-%d",++self::$id);
		} else {
		...
		$post->images = $this->getAttachments($id);
		...
		$this->cache[is_array($id) ? $cid : $id] =& $post;
		return $post;
	}
	...
	public function getAttachments($tags,$order = false,$data = null) {
		...
		$posts = is_array($tags) ? $this->getAttachmentsByID($tags) : $this->getAttachmentsByTags($tags);
	...
	public function media_upload_tabs_filter($tabs) {
		$tabs["pe_quick_media"] = __("Quick Browse",'nietzsche');
		return $tabs;
	}
	...
	public function manage_edit_media_tags_columns_filter($columns) {
		if (isset($columns["posts"])) {
			unset($columns["posts"]);
			$columns["images"] = __("Images",'nietzsche');
		}
		return $columns;
	}
	...
	<div class="pe_wrap">
        <div class="contents clearfix">
			...
			<div class="pe_gallery">
				<div class="pe_output" id="thumbs" data-nonce="<?php echo esc_attr( $nonce ); ?>" <?php esc__pe($multi) ?>>
				</div>
			</div>
		</div>
	</div>
	...
	public function add_meta_boxes_gallery() {
		...
		$fields = array(
						...
						array(
							  ...
							  "type"=>"Text",
							  "section"=>"main",
							  "description" => __("Optional image title.",'nietzsche'),
							  "default"=> ""
							  ),
						"caption" => 
						array(
							  ...
							  "type"=>"Editor",
							  "section"=>"main",
							  "description" => __("Optional image description.",'nietzsche'),
							  "default"=> ""
							  ),
						"link" =>
						array(
							  ...
							  "type"=>"Text",
							  "section"=>"main",
							  "description" => __("Optional image link.",'nietzsche'),
							  "default"=> ""
							  ),
						"video" => 
						array(
						...
						"save" => 
						array(
							  "label"=>__("Accept changes and close the dialog",'nietzsche'),
							  "type"=>"Button",
							  "section"=>"main",
							  "description" => __("Remember to publish -> update the Gallery post for changes to be saved.",'nietzsche'),
							  "default"=> ""
							  )
						);
