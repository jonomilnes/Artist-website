...
class PeThemeAdmin {
	...
	public function __construct(&$master) {
		...
		if (PeGlobal::$config['custom-menus'] != false) {
	...
	public function edit_form_after_title() {
		...
		if (empty($options)) {
			...
			if (count($posts) > 0) {
				...
				//$options = array_merge(array(__("Quick Navigation",'nietzsche') => ""),$options);
			} 
		}
	...
	public function manage_edit_project_columns_filter($cols) {
		$date = $cols["date"];
		unset($cols["date"]);
		$cols["pe_project_tags"] = __("Tags",'nietzsche');
		$cols["date"] = $date;
		return $cols;
	}
	...
	public function add_meta_boxes() {
		$screen = $this->screen;
		if (!empty($screen->base) && $screen->base === "post" && $screen->id === "attachment") {
			$this->master->thumbnail->admin();
		}
	}
	
	public function theme_options() {
		echo '<div class="wrap"><div id="icon-themes" class="icon32"><br /></div><h2>'.__("Theme Options",'nietzsche').'</h2>';
		$this->form->render();
		echo "</div>";
	...
	public function import_demo() {
		if (!$_POST || !@$_POST['nonce'] || !wp_verify_nonce($_POST['nonce'],"pe_theme_import_demo")) {
			$result = json_encode(array("ok"=>false,"message" => __("You don't have sufficient permissions",'nietzsche')));
		} else {
