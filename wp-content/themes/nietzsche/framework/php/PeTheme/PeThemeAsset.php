...
class PeThemeAsset {
	...
	public function __construct(&$master) {
		...
		if (isset($_REQUEST['pe-print-assets'])) {
			$this->minifyJS = $this->minifyCSS = false;
		}
	}
	...
	public function registerCustomCSS() {
		$dir = apply_filters('pe_theme_custom_css_folder','css'); 
		$css = glob(sprintf("%s/%s/pe-theme-*.css",PE_THEME_PATH,$dir));
		$deps = array();
		if (is_array($css) && count($css) > 0) {
			foreach ($css as $file) {
				$file = basename($file); 
				if (strpos($file,'toc') === false) {
					$name = strtr(basename($file,'.css'),'-','_');
					$deps[] = $name;
					$this->addStyle(sprintf('%s/%s',$dir,$file),array(),$name);
				}
			}
