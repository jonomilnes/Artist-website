...
EOT;
	...
	protected function openBodyTag($section,$count) {
		...
		return sprintf('<div id="%s_tab_%s" class="block ui-widget-content %s">',$this->prefixID(),urlencode($this->sectionAttr($section)),$count > 1 ? "ui-tabs-hide" : "");
	...
	public function getHTML() {
		...
		foreach ($sections as $section=>$options) {
			if ($section != "hidden") {
				$tabsHeader[] = sprintf('<li class="ui-state-default %s"><a href="#%s_tab_%s">%s</a><span class="sprite"></span></li>',$count > 1 ? "" : "ui-tabs-selected ui-state-active",$this->prefixID(),urlencode($this->sectionAttr($section)),$section);
