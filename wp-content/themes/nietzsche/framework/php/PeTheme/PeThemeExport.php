		...
		if (defined("PE_EXPORT_DEMO_CONTENT")) {
			if (class_exists('RevSlider')) {
				...
				if (is_array($sliders) && count($sliders) > 0) {
					$style = sprintf('style="width:%spx;margin-right:%spx;"',200,10);
					$nonce = wp_create_nonce("revslider_actions");
					$url = self_admin_url(sprintf('admin-ajax.php?action=revslider_ajax_action&client_action=export_slider&dummy=true&nonce=%s&sliderid=',$nonce));
?>
<h3><?php _e( 'Revolution Sliders' ,'nietzsche'); ?></h3>
<?php foreach ($sliders as $slider): ?>
<a class="button" <?php esc__pe($style); ?> href="<?php echo esc_url($url.$slider->getID()); ?>"><?php echo esc_attr($slider->getTitle()) ?> (<?php echo esc_attr($slider->getAlias()) ?>)</a>
<?php endforeach; ?>
<?php
					}
			}
	...
	public function wp_get_attachment_url_filter($url,$id) {
		$data = wp_get_attachment_metadata($id,true);
		...
		$w = max(1,round($w/128))*128;
		$h = max(1,round($h/128))*128;

		$img = sprintf('%dx%d.png',$w,$h);
		$this->dummyImg++;
		$url = str_replace(home_url(),"",PE_THEME_URL)."/demo/$img";
		//$this->master->debug->log($url);
		return $url;
	}
