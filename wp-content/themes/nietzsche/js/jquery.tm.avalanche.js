...
;(function( $, document, window, undefined ) {
	...
	var AvalancheSlider = function( element, options ){
		...
		var refW = settings.fulscreen ? $( window).width() : settings.fullwidth ? settings.scaleUnder : parseFloat( slider.css( 'width' ) );
		var refH = settings.fulscreen ? $( window ).height() : parseFloat( slider.css( 'height' ) );
		...
		// active slide
		var active = slider.find( 'li:first-child' ).addClass( 'active' );
		...
		// For carousel, set visible to one if 
		...
		$( window ).on( 'resize', function(){
			...
			if( settings.carouselScaleHeight ) slider.css( 'height', 'auto');
			positionCarousel( slider , settings );
		...
		*/
		...
		obj.slideTo = function( index, direction ){
			...
			// Redimention slider and target slide
			// before brought into view
			if( target.children( 'img' ).data( 'loaded' ) ) redimensionSlider( target, settings );
			positionCarousel( slider, settings );
			...
			if( tSupport ){
				...
				if( settings.carousel ){
					...
					slides.animate({ opacity: 1, left: -( tx / visibleSlides ) * direction + 'px' }, settings.speed, settings.easing, function(){
					...
					// Slider is now animating
					slider.data( 'transitioning' , true );
					target.css({ left: sliderW * direction + 'px', visibility: 'visible' }).animate({ opacity: 1, left: 0 + 'px'}, settings.speed, settings.easing, function(){
						animateComplete( slider, settings );
					});
					active.animate({ left: -sliderW * direction + 'px' }, settings.speed, settings.easing );
				}
			}
		};
	...
	function loaded( slide, totalSlides, settings, obj ){
		...
		if( !settings.lazyLoad ) {
			...
			if( totalSlides === slides.children().length ){
				...
				// Clear slider height if carousel
				// height should scale
				if( settings.carouselScaleHeight ){
		...
		active
			...
			.each( function(){
				...
				if( tSupport ){
					...
					easing = caption.data( 'e' ) ? caption.data( 'e' ) : settings.easing;
	...
	*/
	...
	var positionCarousel = function( slider, settings ){
		...
		// Update height
		if( settings.carouselScaleHeight ) slider.css({ height: tallestSlide + 'px' });
	...
	*/
	...
	var redimensionSlider = function( slide, settings ){
		...
		// Size wrapper
		if( settings.carousel && settings.carouselVisible > 1 && settings.carouselScaleHeight ){
			slider.css({
	...
	*/
	...
	var redimensionCaptions = function( slide, settings ){
		...
		slide.find( '.tms-caption' ).each( function(){
			...
			if( !settings.captionScaling || settings.carouselVisible > 1 || $( this ).is( '[data-no-scale]' ) ) return false;
