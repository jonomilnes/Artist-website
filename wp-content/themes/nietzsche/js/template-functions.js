...
jQuery( document ).ready( function($){
	...
	*/
	...
	var page = 'body';
	var launchLink = '[href^="#"]:not([href="#"]), .fade-location, .logo a, .footer-logo a, .navigation a, .side-navigation a, .overlay-navigation a, [class*="portfolio-"] .overlay-link, .post-media .overlay-link, .post-title a, .post-read-more a';
	var excludeLink = '.no-page-fade, .mail-link, .lightbox-link, .contains-sub-menu, [data-sub-menu="true"], .blog .pagination-previous, .blog .pagination-next, .disabled, .scroll-link, .pagination-previous, .pagination-next';
	...
	var templateFunctions = {
		...
		fullScreenSlider: function(){
			...
			$( fullScreenSlider ).each( function() {
				...
				var $this = $( this ),
					...
					interval = $this.attr( 'data-interval' ) ? parseInt( $this.attr( 'data-interval' ), 10 ) * 1000 : 6000;
		...
		pageFade: function(){
			...
			$( 'body' ).on( 'click', launchLink, function( event ) {

				var $this = $( this ),
					hash = $this.prop( 'hash' );

				if ( $this.is( excludeLink ) ) {
				...
				if ( window.location.href.replace( window.location.hash, '' ).replace( '#', '' ) === $this.attr( 'href' ) ) {
					...
					$toScroll.animate({ scrollTop: 0 }, 500 );

					event.preventDefault();

					if ( $body.find( '.overlay-navigation-wrapper.active' ).length ) {

						$body.find( '.overlay-navigation-inner' ).click();

					} else if ( $body.find( '.side-navigation-wrapper.active' ).length ) {

						$( document.getElementById( 'side-nav-hide' ) ).children( 'a' ).click();
				...
				} else if ( this.href && 0 === this.href.indexOf( '#' ) ) {
